...
public class Exercise_SliderChangeEvent extends JFrame {
	...
	public Exercise_SliderChangeEvent() {
		...
		for (int i = 0; i < sliders.length; i++) {
			...
			sliders[i] = new JSlider(JSlider.HORIZONTAL, 0, 255, 128); // 수평모양으로 최소 0이고 최대 255이고 시작값은 128로 시작하는 슬라이더 만듬

			sliders[i].setPaintLabels(true); // 숫자 보이게
			sliders[i].setPaintTicks(true); // 눈금
			sliders[i].setMajorTickSpacing(50); // 중간눈금
			sliders[i].setMinorTickSpacing(10); // 최소눈금
			sliders[i].addChangeListener(new ChangeListener() {
				// 슬라이더가 Change 할때 발생하는 이벤트 감지
				@Override
				public void stateChanged(ChangeEvent e) {
					...
					// 라벨의 배경색을 RGB값으로 바꿔서 컬러픽커처럼 보이게함.
